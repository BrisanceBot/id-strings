<?php
$LANG['auth']['signup'] = 'Registrer deg';
$LANG['generic']['generic'] = 'Du har låst opp den generiske teksten!';
$LANG['generic']['error'] = 'En feil oppstod.';
$LANG['error']['generic'] = 'Ingen feilmelding oppgitt.';
$LANG['error']['loggedIn'] = 'Du er allerede innlogget.';
$LANG['error']['notLoggedIn'] = 'Du er ikke innlogget';
$LANG['auth']['login'] = 'Logg inn';
$LANG['auth']['confirmPassword'] = 'Bekreft passord';
$LANG['auth']['password'] = 'Passord';
$LANG['auth']['email'] = 'E-postadresse';
$LANG['auth']['signedout'] = 'Du var logget ut.';
$LANG['auth']['signout'] = 'Logg ut';
$LANG['name'] = 'Norsk bokmål';
$LANG['error']['incorrectAuth'] = 'Feil e-postaddresse eller passord.';
